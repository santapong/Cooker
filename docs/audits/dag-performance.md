...
**See also:** [`spof-and-database.md`](spof-and-database.md) extends this audit with SPOF/blast-radius analysis (Redis, Postgres, in-memory state, panic cost) and a database-schema review (indexes, foreign keys, migration rollback, secrets-at-rest). [`crash-and-service-quality.md`](crash-and-service-quality.md) is the third companion — a 10-agent fan-out audit covering crash bugs (panics, leaks, races, startup) and service-quality gaps (availability, performance, security, observability, ops readiness) in the rest of the codebase. The three docs share severity ranking conventions and are cross-referenced.
