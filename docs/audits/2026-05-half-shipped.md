...
2. **`HS26-05-02` (User-facing bug)** — **GitHub webhook does not deploy.** `handler/app.go:338` is a `TODO: enqueue a real deploy`. UI shows "auto-deploy" pill on `AppDetailPage`, the rotate-webhook-secret flow works, GitHub signature is verified — and then a 202 fires with no run created. Closest W11 friction: Indie hacker §step 5–6.
3. **`HS26-05-03` (User-facing bug)** — **Three pipeline stage types are no-ops at runtime.** `executeTest`, `executeApproval`, `executeCustom` log and return nil. The toolbar palette offers all three (`PipelineToolbar.tsx:14,17,18`) and the inspector renders config for them (`NodeConfigPanel.tsx:99,152`). A pipeline of "Test → Approval → Custom" succeeds in milliseconds and emits nothing. ML and Enterprise journeys depend on a real Test/Custom; SaaS journey depends on a real Approval.
4. **`HS26-05-04` (User-facing bug)** — **`SettingsPage` registry + cluster CRUD persists nothing.** `handler/registry.go:95-132` and the matching frontend forms accept input, return a `201 {message: "registry added"}`, then `ListRegistryConfigs` returns `[]` on the next paint. Operator-onboarding friction is severe — every Pro-tier persona hits this on first login.
...
### `HS26-05-02` — GitHub webhook deploy is a TODO — **User-facing bug (HIGH)**
...
### `HS26-05-03` — Three pipeline stage types are no-ops — **User-facing bug (HIGH)**

- **Backend:** `executor.go:377-381` (`executeTest`), `460-464` (`executeApproval`), `466-470` (`executeCustom`) all log a single line and `return nil`. The dispatch table at `executor.go:228-238` calls them for `StageTypeTest`, `StageTypeApproval`, `StageTypeCustom`. No script is run, no test container is started, no approval gate blocks.
- **Frontend:** `PipelineToolbar.tsx:13-18` exposes all three in the palette; `NodeConfigPanel.tsx:99-161` renders the config form (test image, custom script, no form for approval). Pipelines run, succeed instantly on these stages, and produce no artifacts.
- **Symptom:** Pipeline editor lets the user wire a "Run tests" stage with `go test ./...`; the stage completes green in 50ms with empty logs. Same for an Approval gate that does not actually pause execution.
- **Backlog:** Not tracked. **Add as P1.** Three distinct backlog items recommended (test runner, approval gate persistence + WS unblock, custom script runner), each ~½ day.
