...
- **F-04** — `pipeline_runs.created_at` existed in the DB schema (`001_initial.up.sql:25`) but had no matching field on `model.PipelineRun`. The Postgres `List` query used `ORDER BY created_at DESC` without selecting the column; the memory `List` returned hash-map order (non-deterministic). Fixed in `claude/w4-f04-created-at` (PR #33 follow-up): `model.PipelineRun` gains `CreatedAt time.Time` (json:`createdAt`); Postgres `Get` and `List` now select the column and `scanRun` scans it; `Create` uses `RETURNING created_at` to populate the field post-insert. Memory `Create` stamps `time.Now()` when the caller leaves `CreatedAt` zero; memory `List` sorts `newest-first` via `sort.Slice`. Two new tests `TestRunStore_ListOrderedByCreatedAt` and `TestRunStore_CreateSetsCreatedAt` lock in the behaviour for the memory impl. No migration required — the column already existed.
