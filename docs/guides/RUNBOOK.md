...
> **SECURITY — `/metrics` is unauthenticated.** It is served on the public app port (8080) with no auth. The secure scrape path is the **cluster-internal Service via the ServiceMonitor**, which never leaves the cluster network — the default install (`ingress.enabled=false` → ClusterIP only) keeps `/metrics` cluster-internal. If you enable the Ingress with the default `/` prefix, `/metrics` becomes **publicly routable**; block it at the ingress (the `ingress.annotations` block in `values.yaml` ships a ready-to-uncomment nginx `configuration-snippet` that does `location = /metrics { deny all; }`). The permanent fix — a dedicated, separately-bound metrics port — is tracked.
