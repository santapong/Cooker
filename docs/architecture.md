...
| **WebSocket Hub** | `internal/server/websocket.go` | Fan-out message broadcasting. Manages client connections per channel. Channels: `pipeline-run:<id>`, `docker-build:<id>`, `kube-watch:<ns>:<resource>`. |
...
| **In-memory store for MVP** | Handlers use in-memory maps for rapid iteration. PostgreSQL store layer is implemented and ready to wire in. Switching requires changing one line in server initialization. |
