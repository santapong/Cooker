.PHONY: all build build-cli clean dev test lint uat-up uat-up-socketproxy uat-up-with-keycloak uat-down uat-logs uat-shell uat-reset test-e2e backup-restore-drill release release-snapshot deploy-docker deploy-docker-down deploy-docker-logs deploy-k8s deploy-k8s-down
...
# --- Easy deploy: single-host docker mode ---
# `make deploy-docker` brings up the production stack (app + postgres +
# redis as separate services) from docker-compose.prod.yml. On first run
# it writes .env.prod with fresh random secrets (DB password, AES-256
# secret key, local-auth JWT signing key) so the production posture boots
# without any manual secret wiring. Re-running reuses the same .env.prod.
#
# Override the served origin before first run if you front Cooker with a
# domain:  COOKER_ALLOWED_ORIGINS=https://cooker.example.com make deploy-docker
...
deploy-docker:
	...
	@echo
	@echo "Cooker (production, docker) ready at http://localhost:8080"
	@echo "  First run: sign up at http://localhost:8080/signup"
	@echo "  logs:  make deploy-docker-logs"
	@echo "  down:  make deploy-docker-down"

deploy-docker-logs:
	$(PROD_COMPOSE) logs -f cooker

# Stops the stack. Keeps volumes (DB data, generated TLS certs) and
# .env.prod so a subsequent deploy-docker resumes with the same state.
# To wipe everything: `docker compose -f docker-compose.prod.yml down -v`.
deploy-docker-down:
	-$(PROD_COMPOSE) down

# --- Easy deploy: kubernetes mode ---
# `make deploy-k8s` installs the Helm chart with the quickstart preset,
# which bundles a Postgres StatefulSet + Redis so `helm install` needs no
# external database. Requires helm + a reachable cluster (kubectl context).
deploy-k8s:
