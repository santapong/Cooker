...
Items that landed in the `claude/uat-ready-*` PR series, PR #6, the `claude/cooker-backlog-readme-com8z` PR (#17), the `claude/complete-p1-backlog-qN4FP` PR, the `claude/finish-backlog-priority-psf4D` PR, the `claude/implement-frontend-design-XVxz2` PR (the Aegis frontend port), the `claude/identify-failure-point-Duy02` PR (#21, the SPOF closeout), the `claude/review-production-rollout-MT3YO` PR (P0 follow-up batch), the `claude/plan-weekly-features-WoB0S` PR (weekly: agent-team complexity + retention CronJob), the `claude/frontend-bundle-split` PR (route-level lazy-load + Vite manualChunks), and the `claude/w2-v0.1.0-scaffolding` PR (v0.1.0 scaffolding — GoReleaser + module path + --version):

### `claude/w2-v0.1.0-scaffolding` — v0.1.0 release scaffolding (pm-brief §3 item #1, shipping-go §1 0-30d)

- ✅ **Module path corrected** — `backend/go.mod` changed from `github.com/cooker-ci/cooker` to `github.com/santapong/cooker`. All import paths in Go source files updated (83 files). Dockerfile `OCI_IMAGE_SOURCE` label, Helm `values.yaml` image repository, raw K8s manifest image ref, golangci.yml errcheck exclusion, and generated swagger docs updated to match. The GitHub repo itself is still named `santapong/Cooker` (capital C) — Go convention requires lower-case module names, so the module path uses `santapong/cooker` (lower-case). See PR body for the case-mismatch question.
- ✅ **`cooker --version` flag** — `backend/cmd/cooker/main.go` now accepts `-version` / `--version` and prints three lines (`version:`, `commit:`, `date:`). Build-time metadata is populated via ldflags (`-X main.version`, `-X main.commit`, `-X main.date`); defaults are `v0.1.0-dev / none / unknown` when built without ldflags. The `server.BuildVersion`, `server.BuildSHA`, `server.BuildTime` vars in `internal/server/version.go` (which feed `GET /api/v1/version`) are now set from the same three vars in `main()`. `Makefile` `build-backend` target now injects ldflags using `git describe` / `git rev-parse` / `date -u`.
- ✅ **`.goreleaser.yaml` scaffolding** — top-level config with `project_name: cooker`, `before.hooks: [go mod tidy]`, `builds` for linux+darwin/amd64+arm64 (windows/zip too), ldflags wired for version/commit/date, `archives` tar.gz/zip, `release.draft: true`. Docker/GHCR and cosign/SBOM blocks are present but commented out, marked TODO for the follow-up signing PR. `make release-snapshot` target added.
