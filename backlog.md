...
**Phase 1 shipped** on `claude/cloud-inventory` — read-only AWS+GCP inventory (compute / managed-K8s / registries) + month-to-date cost panel, see "Closed (recent)". Phases (2) safe lifecycle ops and (3) broader management remain open and each still gets its own ADR. The phase-1 slice is account/project-wide (not yet per-App via cost-allocation tags); narrowing to per-App and adding LB/target-health/storage is folded into a future phase.
...
### `claude/launch-execution` — M0-T3: P6.1 helm-gate hardening

- ✅ **Extended the existing `helm` CI job** (P6.1 already shipped — this is a hardening pass, no parallel job). Added two `helm template` gate-render permutations: `--set metrics.enabled=false --set networkPolicy.enabled=false` (asserts the chart still renders a valid install-able set with the optional integrations off) and `--set metrics.serviceMonitor.enabled=true --set metrics.prometheusRule.enabled=true` (inert until M0-T1 adds the metrics templates, load-bearing thereafter). Both renders are fed into kubeconform.
- ✅ **kubeconform CRD validation** — both kubeconform passes now carry the Datree CRDs-catalog schema location (`-schema-location 'https://raw.githubusercontent.com/datreeio/CRDs-catalog/main/{{.Group}}/{{.ResourceKind}}_{{.ResourceAPIVersion}}.json'`) alongside `-schema-location default`, so off-tree CRDs (e.g. the `monitoring.coreos.com` ServiceMonitor/PrometheusRule from M0-T1) validate against published schemas. `-ignore-missing-schemas` is retained as the fallback so an uncatalogued CRD is skipped rather than hard-failing.
- ✅ **Raw-manifest kubeconform pass** — added a second kubeconform invocation over `deploy/kubernetes/` (the non-Helm parity manifests) so they can't silently drift from valid Kubernetes schema.
- ✅ **`make helm-validate`** — new Makefile target in the Helm section mirroring the CI gates locally: pipes `helm template deploy/helm/cooker` through kubeconform via stdin, then kubeconforms `deploy/kubernetes/`, both with the same CRD schema locations.
- Constraints honoured: Helm chart templates / `values.yaml` untouched (consumed only — M0-T1 owns them); Go 1.25 pin and all other jobs intact; no `continue-on-error`/`--no-verify`/`@main` introduced.

### `claude/cloud-inventory` — read-only cloud inventory & cost panel (OR-2, phase 1)

- ✅ **Read-only provider aggregation** — new `internal/cloudinventory` package: a narrow `Provider` interface (`Name`/`ListResources`/`CostSummary`) with `aws/` and `gcp/` adapters, plus a `Service` that fans out to enabled providers concurrently, aggregates into `model.CloudInventory`, and caches in memory with a TTL (`COOKER_CLOUD_CACHE_TTL`, default 5m). Per-provider error isolation: one cloud failing yields partial results with a per-provider error field, never a 500. Strictly read-only — list/describe/cost APIs only, no mutation surface. No DB persistence (cache only).
- ✅ **AWS via SDK v2** (mirrors `awsm`/`ecs` construction) — EC2 `DescribeInstances`, EKS `ListClusters`/`DescribeCluster`, ECR `DescribeRepositories`, Cost Explorer `GetCostAndUsage` (MTD grouped by service). **GCP via `google.golang.org/api`** (mirrors `gcpsm`/`cloudrun`) — compute `aggregatedList`, container `clusters.list`, artifactregistry `repositories.list`; cost is a labelled zero (GCP MTD spend lives in the BigQuery billing export, not Cloud Billing v1 — deferred, not faked).
- ✅ **API + frontend** — `GET /api/v1/cloud/{inventory,costs}` (read-level), `POST /api/v1/cloud/refresh` (writeRole + rate-limited, busts cache). No provider enabled → 200 `{"enabled":false}`, not an error. New **Cloud** page (route + sidebar): resource table filterable by provider/type, per-provider cost panel, Refresh, per-provider error banners, friendly empty state. `GET /capabilities` advertises `cloudInventory`.
- ✅ **Config + deploy + docs** — `COOKER_CLOUD_*` env vars with production `Validate()` (region/project required when a provider is enabled; creds fall back to IRSA/Workload-Identity/ADC like the secrets backends). Helm chart + raw manifests render the env vars with credentials via `secretKeyRef`. `.env.uat.example`, `docs/guides/UAT.md`, `SECURITY.md` (read-only IAM least-privilege guidance), and `docs/reference/design.md §2.8` updated. Also corrected the stale CLAUDE.md Go-version-pin bullet (repo is on Go 1.25 / x/time v0.15).
- Tests: service (TTL cache, concurrent fan-out, partial-failure isolation), AWS provider (flatten/paginate/cost-grouping against fake SDK clients), GCP provider (flatten + httptest endpoint-override on the real compute client), three handler routes, config validation, and 14 frontend util cases — no real cloud calls in CI.
...
- ✅ **P0.6 — OCI conformance workflow scope** — `pull_request:` trigger removed in commit `a8aa68e` (already on `main` ahead of this branch). Workflow now only runs on `workflow_dispatch` and the weekly `schedule`, treating conformance as a tracked-but-non-blocking signal until a human can pull failing logs.
  - **Root-caused 4 Sep 2026 (local repro against `registry:2`):** two stacked failures. (1) Since PR #153 moved the package to `internal/build/pusher/`, the workflow and `make oci-conformance` still ran `./internal/pusher/...` → "no packages to test". (2) Underneath (failing weekly since ≥14 Jun), `TestPushConformance` applied `mutate.ConfigFile` *after* `mutate.AppendLayers`, wiping `rootfs.diff_ids`; `remote.Write` enumerates layers via diff_ids, so only the config blob was uploaded and the registry answered `MANIFEST_BLOB_UNKNOWN`. Both fixed on `develop` (the test now sets the config first and declares OCI media types so the image-spec check is meaningful). The upstream Ginkgo steps were not re-run locally — the next scheduled run on `main` is the verification; re-enable `pull_request:` once it is green.
