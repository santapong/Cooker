...
export default function PushNode({ data }: NodeProps) {
  ...
  return (
    <div style={{ padding: 12, borderRadius: 8, backgroundColor: '#1e293b', border: `2px solid ${colors[status] || '#475569'}`, minWidth: 180, color: '#f1f5f9' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        <span style={{ width: 24, height: 24, borderRadius: 4, backgroundColor: '#d97706', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 'bold' }}>P</span>
        <span style={{ fontSize: 13, fontWeight: 600 }}>{data.label as string}</span>
      </div>
      <div style={{ fontSize: 11, color: '#94a3b8' }}>
        {data.config && (data.config as Record<string, string>).registry
          ? `Registry: ${(data.config as Record<string, string>).registry}`
          : 'Configure push...'}
      </div>
